...
class CityPreferenceImpl(
    ...
    private fun inputCacheTime() = preferences.getString(CITY_CACHE_TIME, "4")?.toInt() ?: 4

    private fun getLastCacheTime() = preferences.getString(cityLastCache, null)

    override fun updateCacheTimeToNow() =
        preferenceEditor.putString(cityLastCache, ZonedDateTime.now().toString()).apply()

    override fun clearListCacheTime() = preferenceEditor.remove(cityLastCache).commit()
